...
// Create a new match with given players + maxRounds.
app.post("/match", async (req, res) => {
  const matchData = await req.body
  const players = matchData.players
  const maxRounds = matchData.rounds
})

// Fetch match + all rounds + winner.
app.get("/match/:id", (req, res) => {

})

// Add a round result (heroRoll, villainRoll, winner) to a match.
